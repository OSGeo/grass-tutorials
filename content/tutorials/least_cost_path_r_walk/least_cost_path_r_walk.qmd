...
format: 
  html:
    ...
    code-fold: false
page-layout: article
categories: [intermediate, advanced, GUI, raster]
engine: jupyter
execute: 
  eval: false
jupyter: python3
...
::: {.callout-note}
This tutorial uses one of the standard GRASS demo data: flagstaff_arizona_usa.
We will refer to place names in that data set, but it can be completed with
any of the standard demo data sets for any region. We will use the
...
A *cost surface* is a raster where each cell has a cost value,
indicating the difficulty of traversing that area.

A *cumulative cost surface* shows the total accumulated cost of moving
from a starting point to a location, often used to find the least-cost path.
...
::: g-col-6
![Cumulative cost surface](images/GRASS_movement_0.png)
:::
:::::

## How to model movement with a cost surface

::::: grid
::: g-col-6

1. Start with a *digital elevation model (DEM)* to determine movement costs.
2. Create a *friction map* with a value of 0 (or other vallues for additional
   movement costs).
...
::: g-col-6
![DEM](images/GRASS_movement_1.png)
:::
:::::
...
::::: grid
::: g-col-6
Add *elevation* raster, right click at the layer in Layer Manager and select
*Set computational region from selected map*.

In Raster Map Calculator (top toolbar) fill in the output map name *friction0*
and 0 in the expression field.
:::

::: g-col-6
![Raster map calculator](images/GRASS_movement_2.png)
:::
:::::
...
::: g-col-6
![Attribute Table Manager for hospitals vector points map](images/GRASS_movement_3.png)  

![Table row context menu](images/GRASS_movement_4.png){width="60%"}
:::
:::::
...
![](images/GRASS_movement_5.png)

### Compute cumulative cost surface
...
Enter the input and output parameters: **elevation**=*elevation*, **friction**=*friction0*, and **output**=*FMC_cost_seconds*.
:::

::: g-col-6
![r.walk tool: select input maps](images/GRASS_movement_6.png)
:::
:::::

::::: grid
::: g-col-6
Enter the name of a directions map **outdir**=*FMC_directions* for creating a least cost path.  
:::

::: g-col-6
![r.walk tool: outdir parameter](images/GRASS_movement_7.png)
:::
:::::

::::: grid
::: g-col-6
Enter the start point (**start_points**=*FMC*).
:::
::: g-col-6
![r.walk tool: start_points parameter](images/GRASS_movement_8.png){fig-align="left" width="100%"}
:::
::::::
...
:::
::: g-col-6
![r.walk tool: stop_points parameter](images/GRASS_movement_9.png)
:::
:::::
...
:::
::: g-col-6
![r.walk tool: walking parameters](images/GRASS_movement_10.png)
:::
:::::

::::: grid
::: g-col-6
Recommended: select *knight's move* for more accurate cost and direction.
:::
::: g-col-6
![r.walk tool: select *knight's move*](images/GRASS_movement_11.png)
:::
:::::
...
![Displaying the cost surface over a shaded relief map using the [d.shade](https://grass.osgeo.org/grass-stable/manuals/d.shade.html) tool and a relief map of elevation made with [r.relief](https://grass.osgeo.org/grass-stable/manuals/d.shade.html).](images/GRASS_movement_12.png)

### Movement across a cost surface
...
::::: grid
::: g-col-6
You can transform the walking time in seconds to hours by dividing
the map by 3600 using the map calculator with expression `FMC_cost_seconds / 3600`.
:::
::: g-col-6
![](images/GRASS_movement_13.png)
:::
::::::
...
![Shaded area represents terrain reached within a 2 hour walk from FMC](images/GRASS_movement_14.png)
...
![Start and end point](images/GRASS_movement_16.png)
...
::::: grid
::: g-col-6
We need to use the directions map **input**=*FMC_directions*
we made when we created the cost surface.
:::
::: g-col-6
![](images/GRASS_movement_17.png)
:::
::::::

::::: grid
::: g-col-6
The coordinates of the hiker **start_coordinates**=*477476,13914951*
(we could also use a pre-defined vector point).
:::
::: g-col-6
![](images/GRASS_movement_18.png)
:::
:::::

:::: grid
::: g-col-6
And specify the name of the output LCP **vector_path**=*LCP_cumulative*.
:::
::: g-col-6
![](images/GRASS_movement_19.png)
:::
:::::
...
![Resulting least cost path](images/GRASS_movement_20.png)

## Adding a friction surface to movement costs

A **friction surface** can be used to incorporate other factors than
...
We can use the land use map (*landuse*) to create a friction surface
of the amount of extra time it would take to walk through different kinds of land cover.  

![Reclassification of *landuse* to show major land cover types](images/GRASS_movement_21.png)

### Reclassification of landuse to friction

A standard walking speed across flat terrain is about 5 km/hr = 0.72 sec/m.
We might then estimate that it might take an additional:
...
We can reclassify landuse to create a new friction map with these values using
[r.reclass](https://grass.osgeo.org/grass-stable/manuals/r.reclass.html),
which lets us assign new values to categories in an existing raster map.

::: {.panel-tabset group="language"}

## GUI

Tool [r.reclass](https://grass.osgeo.org/grass-stable/manuals/r.reclass.html)
...
- The original raster **input**=*landuse*
- A name for the reclassification **output**=*friction_landcover*
- The reclassification rules (can also be in a text file):
...
![The new friction surface map](images/GRASS_movement_23.png)

### Modifying a cost surface with a friction surface
...
See [Compute cumulative cost surface](#cost-surfaces)
...
![Cumulative cost surface with landcover friction](images/GRASS_movement_24.png)
...
It takes more time to reach FMC if the hiker has to navigate
dense vegetation as well as terrain.
...
![The original LCP with terrain only is shown by the blue line. The LCP with a landcover friction surface is shown by the heavier yellow line](images/GRASS_movement_25.png)
