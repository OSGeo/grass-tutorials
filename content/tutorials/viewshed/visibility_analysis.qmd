...
project = Path.home() / "grassdata" / "nantahala"
...
- `ned_release="all"` is needed here because the default `"current"` returns no tiles
  for this footprint.
- `title_filter="20220725"` pins one published vintage so the numbers below stay
  reproducible. Drop it to always fetch whatever NED is current for the area.
:::
...
tools.v_in_ascii(
    input=io.StringIO(towers_csv), output="towers", separator="pipe",
...
```{python}
print(tools.r_report(map="wayah_ground", units="k", flags="n").stdout)
print(tools.r_report(map="wayah_tower", units="k", flags="n").stdout)
```
...
print(tools.r_report(map="wayah_smoke", units="k", flags="n").stdout)
```
...
print(tools.r_report(map="blind", units="k", flags="n").stdout)
print(tools.r_report(map="recovered", units="k", flags="n").stdout)
```
...
tools.v_in_ascii(input=io.StringIO(f"{cx:.0f}|{cy:.0f}"), output="candidate",
                 separator="pipe", columns="x double precision, y double precision",
                 x=1, y=2)
