...
towers was a visibility problem: where should a tower go so it can see as much of the forest
as possible?
...
download the terrain around them from the USGS. We work in a new GRASS project in UTM zone 17N (EPSG:32617)
and fetch 1 arc-second (about 30 m) elevation from the USGS National Map with
...
session = gj.init("~/grassdata", "nantahala", "PERMANENT")
tools = Tools()
...
This pulls one NED tile (about 58 MB) and takes a couple of minutes, so the cell is
not instant. A few `r.in.usgs` details worth knowing:

- Add `flags="i"` first to preview the tiles and total size without downloading.
- `ned_release="all"` is needed here because the default `"current"` returns no tiles
  for this footprint.
- `title_filter="20220725"` pins one published vintage, which keeps the download and
  the areas reported below reproducible. Drop it to fetch whatever is current.
:::
...
coordinates (converted from the published latitude and longitude to UTM 17N), record
the height of each tower's cab above the ground, and read the summit elevation from
the DEM with
...
[d.text](https://grass.osgeo.org/grass-stable/manuals/d.text.html) at its position in
the current region.
...
def label_towers(m, only=None):
    """Draw each tower's name on map m, placed from its coordinates."""
    r = gs.parse_command("g.region", flags="g")
    w, e, s, n = float(r["w"]), float(r["e"]), float(r["s"]), float(r["n"])
    for rec in json.loads(tools.v_db_select(map="towers", format="json").stdout)["records"]:
        if only and rec["name"] not in only:
            continue
        x_pct = (rec["x"] - w) / (e - w) * 100
        y_pct = (rec["y"] - s) / (n - s) * 100
        m.d_text(text=rec["name"], at=(x_pct + 1.5, y_pct), color="black",
                 size=2.6, bgcolor="white")

study = gj.Map(width=600)
study.d_rast(map="shaded_elev")
study.d_vect(map="towers", icon="basic/triangle", size=13,
             fill_color="white", color="black", width=2)
label_towers(study)
study.d_legend(raster="elevation", at=(4, 28, 88, 92), flags="b", title="m")
study.d_barscale(flags="n", at=(2, 5), bgcolor="white")
study.show()
```
...
range, apply Earth-curvature correction with `-c` (see the callout below).
...
A small helper turns a boolean viewshed into a visible area in km², reading the cell
size from the current region so it matches the imported grid:

```{python}
def visible_km2(viewshed):
    res = gs.region()["nsres"]
    stats = tools.r_stats(input=viewshed, flags="cn").stdout
    cells = sum(int(line.split()[1]) for line in stats.splitlines()
                if line.split()[0] == "1")
    return cells * res * res / 1e6

print(f"standing on the summit: {visible_km2('wayah_ground'):.0f} km2")
print(f"from the 16 m tower:    {visible_km2('wayah_tower'):.0f} km2")
```

Standing on the bald, the lookout can see about 165 km². From the tower cab it is
about 277 km², nearly 70% more for 16 m of extra height. That gain is what made the
towers worth building. We draw the ground-level viewshed in dark red and the extra area the tower
adds in orange, zooming in around Wayah Bald.
...
gs.write_command("r.colors", map="ground_vis", rules="-", stdin="1 178:24:43")
gs.write_command("r.colors", map="tower_gain", rules="-", stdin="1 239:138:98")

tools.g_region(n=3908000, s=3884000, w=252000, e=284000, res=30)   # zoom to Wayah Bald

height_map = gj.Map(width=600)
...
height_map.d_vect(map="towers", where="name='Wayah Bald'", icon="basic/triangle",
                  size=13, fill_color="white", color="black", width=2)
label_towers(height_map, only=["Wayah Bald"])
height_map.d_barscale(flags="n", at=(2, 5), bgcolor="white")
height_map.show()

tools.g_region(raster="elevation")   # restore the full region for analysis
```
...
A lookout scans for smoke rising above the canopy rather than for the ground itself.
A column of smoke a few meters tall can clear a ridge that hides the ground behind
it. `r.viewshed` models this with `target_elevation`, the height of the target above
the ground. We set it to 15 m, a modest smoke column.
...
print(f"ground visible:   {visible_km2('wayah_tower'):.0f} km2")
print(f"smoke detectable: {visible_km2('wayah_smoke'):.0f} km2")
```

Allowing for a 15 m smoke column raises the detectable area from 277 km² to about
412 km², nearly 50% more. In the map below the dark red is now the tower's full
terrain view (the 277 km² baseline) and orange is the extra area where the ground is
hidden but a 15 m smoke column would still be visible.
...
gs.write_command("r.colors", map="terrain_vis", rules="-", stdin="1 178:24:43")
gs.write_command("r.colors", map="smoke_gain", rules="-", stdin="1 253:184:99")

tools.g_region(n=3908000, s=3884000, w=252000, e=284000, res=30)   # zoom to Wayah Bald

smoke_map = gj.Map(width=600)
...
smoke_map.d_barscale(flags="n", at=(2, 5), bgcolor="white")
smoke_map.show()
...
One tower leaves gaps. Towers with overlapping views fill in each other's blind
spots. We loop over the three towers, compute a smoke-detection viewshed for each using its own cab height, and add
the boolean maps with
...
```{python}
towers = json.loads(tools.v_db_select(map="towers", format="json").stdout)["records"]
...
per-class areas, and its JSON output is easy to turn into a table.

```{python}
import pandas as pd

report = json.loads(
    tools.r_report(map="coverage", units=["k", "p"], format="json").stdout
)
rows = []
...
```{python}
gs.write_command("r.colors", map="coverage", rules="-",
                 stdin="1 254:224:139\n2 252:141:89\n3 213:62:79")

coverage_map = gj.Map(width=600)
coverage_map.d_rast(map="relief")
coverage_map.d_rast(map="coverage")
coverage_map.d_vect(map="towers", icon="basic/triangle", size=13,
                    fill_color="white", color="black", width=2)
label_towers(coverage_map)
coverage_map.d_legend(raster="coverage", at=(4, 16, 88, 92), flags="b",
                      use=[1, 2, 3], title="towers")
coverage_map.d_barscale(flags="n", at=(2, 5), bgcolor="white")
coverage_map.show()
```
...
The gap map lets us test candidate sites for a fourth tower. A natural candidate is the highest summit that no current tower can see, since high ground has
the best chance of a long view. We isolate the blind area, find its highest cell, and
test a tower there.
...
stats = tools.r_stats(input="blind_elev", flags="gn").stdout.strip().splitlines()
cells = [(float(x), float(y), float(z)) for x, y, z in (s.split() for s in stats)]
cx, cy, cz = max(cells, key=lambda c: c[2])
...
print(f"blind area:        {visible_km2('blind'):.0f} km2")
print(f"recovered by tower: {visible_km2('recovered'):.0f} km2")
```

The best-placed new tower recovers only about 28 km² of the 870 km² blind area, less
than 4% of it. The gap map shows why: what remains unseen is mostly the valley
bottoms between the ridges, and no ridgetop tower can see down into a ravine. Beyond a handful of well-sited towers, adding more brings
sharply diminishing returns. Fire agencies eventually turned to aircraft patrols and,
later, camera and satellite detection for exactly this reason.
...
site_map.d_vect(map="towers", icon="basic/triangle", size=13,
                fill_color="white", color="black", width=2)
site_map.d_vect(map="candidate", icon="basic/star", size=18,
                fill_color="yellow", color="black", width=2)
site_map.d_barscale(flags="n", at=(2, 5), bgcolor="white")
site_map.show()
```
...
- A rising smoke column (`target_elevation`) can be detected over far more ground
  than is directly visible.
