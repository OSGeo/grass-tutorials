...
in GRASS 8.5—specifically, the `MaskManager` and `RegionManager`—which simplify
region and mask handling in parallel workflows.
...
def compute(huc12):
    ...
    with gs.RegionManager(raster=f"ned_{huc12}"):
        ...
        with gs.MaskManager(mask_name=f"basin_{huc12}"):
            ...
            gs.mapcalc(f"detachment_coef_{huc12} = 0.001")
            gs.mapcalc(f"shear_stress_{huc12} = 0.01")
            region = gs.parse_command("g.region", flags="p", format="json")
            gs.run_command(
                "r.sim.sediment",
                elevation=f"ned_{huc12}",
                water_depth=f"depth_{huc12}",
            ...
            erosion = gs.parse_command(
                "r.univar", map=f"erosion_{huc12}", format="json", flags="e"
            )
            return {
                "huc12": huc12,
