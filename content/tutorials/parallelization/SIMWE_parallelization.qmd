...
date: 2025-06-25
date-modified: today
categories: [Python, advanced, paralellization, hydrology, erosion]
description: >
...
In this tutorial we will analyze overland water flow and erosion and deposition patterns along a Yadkin river in North Carolina, USA.
We will use SIMWE model for the flow and erosion modeling with simplified runoff inputs based on [National Land Cover Dataset](https://www.mrlc.gov/data) (NLCD).
We will use hydrologic units ([HUC 12](https://www.usgs.gov/national-hydrography/access-national-hydrography-products), subwatersheds, approximately 100 km2) along the river to divide our computation and parallelize it.
Finally, we will use National Elevation Dataset (NED) at 1/3 arc-second (approximately 10 m) resolution.
...
Download link to the NLCD data can be found at [mrlc.gov/data](https://www.mrlc.gov/data). We will download it to the current working directory.
...
nlcd_filename, headers = urllib.request.urlretrieve(
    url, os.path.join(os.getcwd(), "NLCD.tif")
)
```

Create a GRASS project using the coordinate reference system (CRS) of the NLCD dataset, and link the NLCD raster with [r.external](https://grass.osgeo.org/grass-devel/manuals/r.external.html). This command creates a virtual raster without importing the full dataset, allowing us to work with just the small sections needed from the larger, nationwide NLCD file.
...
subwatersheds = gs.parse_command("v.db.select", map="river_basins", format="json")[
    "records"
]
...
```{python}
gs.run_command("g.region", vector=f"basin_{huc12}")
```
...
```{python}
gs.run_command(
    "r.proj", project="nlcd", mapset="PERMANENT", input="nlcd", output=f"nlcd_{huc12}"
)
...
```{python}
gs.run_command(
    "r.recode", input=f"nlcd_{huc12}", output=f"mannings_{huc12}", rules="mannings.txt"
)
```
...
```{python}
gs.run_command(
    "r.recode", input=f"nlcd_{huc12}", output=f"runoff_{huc12}", rules="runoff.txt"
)
```
...
```{python}
gs.run_command(
    "r.slope.aspect", elevation=f"ned_{huc12}", dx=f"dx_{huc12}", dy=f"dy_{huc12}"
)
```
...
gs.run_command(
    "r.grow", input=f"erdep_tmp_{huc12}", output=f"erdep_{huc12}", radius=-2.01
)
...
erosion = gs.parse_command("r.univar", map=f"erosion_{huc12}", format="json")
```

```{python}
#| code-fold: true
#| code-summary: erosion
{'n': 181072,
 'null_cells': 418835,
 'cells': 599907,
...
making it possible to have custom mask for the current process. This feature is available only since GRASS 8.5.
    ...
    env = os.environ.copy()
    with gs.MaskManager(mask_name=f"basin_{huc12}", env=env):
        # Run actual computation with active mask.
        gs.run_command(..., env=env)
    ```
...
making it possible to have custom region for the current process. This feature is available only since GRASS 8.5.
    ...
    # create a copy of environment
    env = os.environ.copy()
    with gs.RegionManager(vector=f"basin_{huc12}", env=env):
        # Run actual computation in the specified region.
        gs.run_command(..., env=env)
    ```

The environment, unique for each process, is passed through the `env` parameter to all tools. Results are collected and stored in a JSON file for further analysis.
...
def compute(huc12):
    ...
    with gs.RegionManager(raster=f"ned_{huc12}", env=env):
        gs.run_command(
            "v.to.rast",
        ...
        with gs.MaskManager(mask_name=f"basin_{huc12}", env=env):
            ...
            region = gs.parse_command("g.region", flags="p", format="json", env=env)
            gs.run_command(
                "r.sim.sediment",
                elevation=f"ned_{huc12}",
                water_depth=f"depth_{huc12}",
            ...
            erosion = gs.parse_command(
                "r.univar", map=f"erosion_{huc12}", format="json", env=env
            )
            return {
                "huc12": huc12,
...
if __name__ == "__main__":
    basins = gs.parse_command("v.db.select", format="json", map="river_basins")[
        "records"
    ]
...
df["normalized_erosion"] = df.erosion_mean / max(
    max(df.erosion_mean), max(df.deposition_mean)
)
df["normalized_deposition"] = df.deposition_mean / max(
    max(df.erosion_mean), max(df.deposition_mean)
)
df
```
...
metric = "normalized_deposition"
color_scale = cm.linear.OrRd_09.scale(min(gdf[metric]), max(gdf[metric]))
color_scale.caption = "Min-max normalized erosion"
...
tooltip = folium.GeoJsonTooltip(
    fields=["huc12", "normalized_erosion", "normalized_deposition"],
    aliases=["HUC12", "Normalized erosion:", "Normalized deposition:"],
    localize=True,
    sticky=False,
    labels=True,
    style="""
