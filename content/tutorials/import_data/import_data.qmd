...
It is also a **template** for assembling an equivalent dataset for another area. The
reusable part is the list of layers and what each one holds, not the particular names
used here - a course taught in another language would localize those.
Material written against the North Carolina dataset
(tutorials, course exercises, scripts) can be run with the derived localized datasets.
To see all the layers, download the [notebook](raleigh_northcarolina_usa_epsg6542.ipynb)
...
only: the `r.in.usgs` and `r.in.ssurgo` add-ons, the pygris package (US Census), NC
OneMap and Wake County GIS. Several are state or county level and have no national
equivalent even within the US.

The *methods* transfer anywhere, and so do the tools doing the actual importing:
`r.import`, `v.import`, `r.external`, `r.proj`, and `v.in.ags` for any ArcGIS REST
service, which national and municipal agencies run all over the world. The workflow and
the choice of layers carry over to another area; the list of sources does not.
:::
...
```{python}
tools.g_region(n=228500, s=215000, w=629000, e=653000, res=10)
tools.g_region(flags="s")
```

These particular numbers represent just a window over Raleigh that we
...
When importing raster data, you can specify `extent="region"` that clips to the
computational region instead of importing the whole file, and `resolution="value"` with
`resolution_value=`, which pins the cell size instead of the resolution estimated during
reprojection.
...
a national raster to read one small window out of it is more work than the job needs.

A lighter approach is a **second project in the data's own CRS**, where the file is
...
respects the computational region, only our window is ever reprojected:
...
For several major data providers, someone has already written the GRASS tool. These
...
The nice property they typically share: they take the computational region as the area of
...
With elevation in place, the roads we imported in Method 1 have something to sit on:

```{python}
road_map = gj.Map()
road_map.d_rast(map="elevation")
...
![Roads styled by class over USGS elevation.](images/roads.webp)

## ArcGIS REST services

An enormous amount of government data is published as ArcGIS MapServer and
...
The water bodies layer mixes lakes with canals and swamps, so we filter with
[v.extract](https://grass.osgeo.org/grass-stable/manuals/v.extract.html)
or alternatively filter with v.in.ags options directly:
...
SSURGO comes as survey-area polygons that spill well beyond our window. To trim them
to the region exactly, turn the region itself into a vector with
[v.in.region](https://grass.osgeo.org/grass-stable/manuals/v.in.region.html) and
intersect. The `olayer="0,1,0"` keeps only the soil attributes and drops the
(meaningless) attributes of the clipping box:

```{python}
tools.v_in_region(output="region", type="area")
tools.v_overlay(ainput="soils_", binput="region", output="soils",
                olayer="0,1,0", operator="and")
tools.g_remove(name="region,soils_", flags="f", type="vector")
```

Alternatively use [v.clip](https://grass.osgeo.org/grass-stable/manuals/v.clip.html)
with -r flag.
...
![Census blocks over land cover.](images/census_blocks.webp)
...
Here we ask the Planetary Computer for Landsat Collection 2 Level-2 scenes over our
...
our study area, at some awkward value near 30 m. We want neither: `extent="region"`
clips each band to our window, and `resolution="value"` with `resolution_value=30` pins
the cell size to a round 30 m, the scene's native resolution, rather than resampling it
to the region's 10 m.
...
def grass_timestamp(iso_string):
    ...
    formatted = dt.strftime("%d %b %Y %H:%M:%S.%f")[:-4]
...
The API answers for a rectangle, so the result overhangs the state. We clip it against
the state boundary with the same `v.overlay` idiom used for the soils, and colour it
from the RGB column the service provides:
...
tools.v_overlay(ainput="geology_", binput="nc", output="geology",
                olayer="0,1,0", operator="and")
...
| `census_blocks`, `zipcodes`, `municipal_boundary` | [TIGER/Line, US Census Bureau](https://www.census.gov/geographies/mapping-files/time-series/geo/tiger-line-file.html) | [TIGER/Line 2025 technical documentation](https://www2.census.gov/geo/pdfs/maps-data/data/tiger/tgrshp2025/TGRSHP2025_TechDoc_Ch1.pdf) |
