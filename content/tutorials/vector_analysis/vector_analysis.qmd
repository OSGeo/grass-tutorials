...
# Attribute columns
print(tools.v_info(map="schools_wake", flags="c").text)
```
...
| NAMESHORT | GLEVEL | ADDRCITY | CORECAPACI |
|---|---|---|---|
...
middle, `H` for high, and so on), which we use next to select schools by
...
overview.d_barscale(flags="n", at=(60, 4), bgcolor="white")
overview.show()
```
...
To keep just the **elementary schools** (`GLEVEL='E'`), use
...
print(tools.v_info(map="elementary", flags="t").text)  # points=95
```
...
attr_map.d_barscale(flags="n", at=(60, 4), bgcolor="white")
attr_map.show()
```
...
A **buffer** is an area of a given radius around features. Here we buffer the major
...
buffer_map.d_barscale(flags="n", at=(60, 4), bgcolor="white")
buffer_map.show()
```
...
print(tools.v_info(map="schools_near", flags="t").text)  # points=15
```

Only **15 out of the 95 elementary schools** lie within 500 m of a major road, a
...
near_map.d_barscale(flags="n", at=(60, 4), bgcolor="white")
near_map.show()
```
...
overlay_map.d_barscale(flags="n", at=(60, 4), bgcolor="white")
overlay_map.show()
```
...
Raleigh leads by a wide margin because it is the largest ZIP code and is crossed by
the most major roads.
...
choropleth.d_barscale(flags="n", at=(60, 4), bgcolor="white")
choropleth.show()
```
...
Using the North Carolina schools, roads, and ZIP codes, we have worked through the
