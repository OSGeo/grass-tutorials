...
`schools_wake` has 167 point features. To peek at a few attribute values, use
[v.db.select](https://grass.osgeo.org/grass-stable/manuals/v.db.select.html):

```{python}
print(tools.v_db_select(map="schools_wake", columns="NAMESHORT,GLEVEL",
                        where="GLEVEL='H'").stdout)
```

Let's start with an overview map: the ZIP code areas, the major roads, and every
school. In [grass.jupyter](https://grass.osgeo.org/grass-stable/manuals/libpython/grass.jupyter.html),
a `Map` object collects display layers and renders them together.

```{python}
overview = gj.Map(width=800)
...
overview.d_barscale(flags="n", at=(4, 6))
overview.show()
```
...
middle, `H` for high, and so on). To keep just the **95 elementary schools**, use
...
attr_map.d_barscale(flags="n", at=(4, 6))
attr_map.show()
```
...
buffer_map.d_barscale(flags="n", at=(4, 6))
buffer_map.show()
```
...
near_map.d_barscale(flags="n", at=(4, 6))
near_map.show()
```
...
overlay_map.d_barscale(flags="n", at=(4, 6))
overlay_map.show()
```
...
choropleth.d_barscale(flags="n", at=(4, 6))
choropleth.show()
```
...
choropleth into a finished, annotated map, continue with
